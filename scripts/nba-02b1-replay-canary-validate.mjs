...
check('final status is explicit migration gate', cert.status === 'NBA_02B1_REPLAY_CANARY_DB_MIGRATION_AUTHORIZATION_REQUIRED')
...
check('schema blocker proven', cert.schemaIsolation.selectable === false && cert.schemaIsolation.error.includes('prediction_origin'))
...
check('doc records migration gate', doc.includes('NBA_02B1_REPLAY_CANARY_DB_MIGRATION_AUTHORIZATION_REQUIRED') && doc.includes('prediction_history.prediction_origin'))
