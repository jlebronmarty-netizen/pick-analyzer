...
check('id rejected from persistence payload', audit?.unsupportedProductionColumns?.includes('id'))
...
check('same game import is idempotent', audit?.idempotencyFixtures?.sameGameTwiceSameGameId === true)
check('corrected score keeps same identity', audit?.idempotencyFixtures?.correctedScoreSameGameId === true)
check('different game uses distinct identity', audit?.idempotencyFixtures?.differentGameDistinctGameId === true)
...
check('deterministic text never sent to UUID id', audit?.idempotencyFixtures?.deterministicTextSentToUuidId === false)
