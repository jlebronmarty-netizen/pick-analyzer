...
check('Mission Control status preserves MC-01 certification', status.mc01?.status === 'PRODUCTION_CERTIFIED' || (status.currentMission?.id === 'MC-01' && ['CONDITIONAL_PASS', 'PRODUCTION_CERTIFIED'].includes(status.currentMission?.state)))
check('Mission Control records MC-02 progression only after MC-01', queue.includes('MC-02 is READY but was not started') || queue.includes('MC-02 | Multi-Sport Data Readiness | MULTI_SPORT_DATA | P1 | PRODUCTION_CERTIFIED'))
...
check('runtime service reports MC-01 certified or conditional', service.includes("id: 'MC-01'") && (service.includes("state: 'CONDITIONAL_PASS'") || service.includes("state: 'PRODUCTION_CERTIFIED'")))
check('runtime service uses MC-01 or later certified current mission', service.includes('currentMission: queue[1]') || service.includes('currentMission: queue[2]'))
check('runtime service excludes completed missions from next mission search', service.includes("!['MC-00', 'MC-01'].includes(mission.id)") || service.includes("!['MC-00', 'MC-01', 'MC-02'].includes(mission.id)"))
