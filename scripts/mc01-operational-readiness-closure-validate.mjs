...
check('MC-01 certification is conditional', certification.status === 'CONDITIONAL_PASS')
check('Mission Control status tracks MC-01 current mission', status.currentMission?.id === 'MC-01' && status.currentMission?.state === 'CONDITIONAL_PASS')
check('Mission Control keeps MC-02 out of execution', queue.includes('Do not begin MC-02'))
check('MC-STOP-005 remains active', status.mc01?.activeStopCondition === 'MC-STOP-005' && certification.activeStopConditions?.[0]?.id === 'MC-STOP-005')
...
check('Master Roadmap mentions MC-01 conditional', roadmap.includes('MC-01 Operational Readiness Closure') && roadmap.includes('CONDITIONAL_PASS'))
