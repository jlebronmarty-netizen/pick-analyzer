...
check('canary schema selectable', canary.schemaIsolation.selectable === true)
check('canary odds nullability gate resolved', canary.status === 'NBA_02B1_REPLAY_CANARY_PERSISTED_ISOLATED' && canary.oddsNullabilityContract.migrationRequired === false)
...
check('canary readback persisted and isolated', canary.persistenceDecision.readbackCount === 96 && canary.persistenceDecision.wrongOriginCount === 0)
