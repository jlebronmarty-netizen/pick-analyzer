...
if (!['dry-run', 'write'].includes(mode)) {
  console.error(JSON.stringify({ success: false, error: 'Invalid mode. Use --mode=dry-run or --mode=write.' }, null, 2))
  process.exit(1)
}

if (mode === 'write' && process.env.NBA_CURRENT_ERA_SHADOW_WRITE_AUTHORIZED !== 'true') {
  console.error(
    JSON.stringify(
      {
        success: false,
...
const result = await runNbaCurrentEraShadowCanary({ mode })
