...
const contractPath=new URL('../../contracts/MLB_F5_STRICT_PREGAME_COUNT_V1.json',import.meta.url)
...
const teamRows=g=>[
  {y:g[3],raw:[g[8],g[9],g[10],g[11],g[12],...g.slice(25,32)],home:1},
  {y:g[4],raw:[g[13],g[14],g[15],g[16],g[17],...g.slice(18,25)],home:0}
]

function standardizer(games){
  const rs=games.flatMap(teamRows),k=12,means=Array(k).fill(0),sds=Array(k).fill(0)
...
export function fit(games){
  ...
  const rs=games.flatMap(teamRows),s=standardizer(games),p=14,lambda=contract.fitting.l2_lambda
...
export function evaluate(input){
  ...
  for(const r of rows){
    assert(r.length===32,'SOURCE_SCHEMA')
    ...
    for(const v of r.slice(8,32))assert(Number.isFinite(v),'FEATURE_VALUE')
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
  assert(process.argv[2]&&process.argv[3],'Usage: node evaluate_mlb_f5_strict_pregame_count_v1.mjs input.json output.json')
