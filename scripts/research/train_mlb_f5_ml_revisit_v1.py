...
SEED=20260919
EDGE_URL="https://ynuocvexviorgdjrfthw.supabase.co/functions/v1/mlb-f5-ml-revisit-github-export-temp"
EDGE_CONTRACT="MLB_F5_ML_REVISIT_GITHUB_EXPORT/1.0.0"
...
OUT=Path("artifacts/research/mlb_f5_ml_revisit_v1_result.json")
FREEZE=Path("artifacts/research/mlb_f5_ml_revisit_v1_frozen_candidate.json")
...
def fetch_rows(token:str)->list[dict[str,Any]]:
    ...
    while True:
        ...
        if not resp.ok:
            raise RuntimeError(f"F5_EXPORT_HTTP_{resp.status_code}:{resp.text[:800]}")
        p=resp.json()
        checks=[
        ...
        if bad: raise RuntimeError("F5_EXPORT_INVALID:"+",".join(bad))
        if expected is None: expected=int(p["totalRows"])
        if int(p["totalRows"])!=expected: raise RuntimeError("F5_EXPORT_TOTAL_CHANGED")
        batch=p.get("rows") or []
        rows.extend(batch);offset+=len(batch)
        if not batch or offset>=expected: break
    if expected is None or len(rows)!=expected:
        raise RuntimeError(f"F5_EXPORT_COUNT:{len(rows)}:{expected}")
    return rows
...
def build_frame(rows:list[dict[str,Any]])->tuple[pd.DataFrame,int]:
    ...
    if (df["_game_date"]>MAX_HIST_DATE).any(): raise RuntimeError("F5_HIST_CUTOFF_BREACH")
    if (df["_game_date"]==pd.Timestamp("2026-09-19")).any(): raise RuntimeError("F5_QUARANTINE_BREACH")
    if (df["_game_date"]>=FORWARD_MIN_DATE).any(): raise RuntimeError("F5_FORWARD_BREACH")
    if not (df["_development_class"]=="HISTORICAL_SEEN_DEVELOPMENT").all():
        raise RuntimeError("F5_CLASS_BREACH")
    if not df["_research_only"].fillna(False).astype(bool).all():
        raise RuntimeError("F5_RESEARCH_BREACH")
    cutoff=pd.to_datetime(df.get("feature_cutoff_date"),errors="coerce")
    if cutoff.notna().any() and (cutoff>=df["_game_date"]).any():
        raise RuntimeError("F5_STRICT_PRIOR_BREACH")
...
def main():
    ...
    if not token: raise RuntimeError("F5_REVISIT_OIDC_MISSING")
    ...
    for month,start_s,end_s in FOLDS:
        ...
        if tr.sum()<300 or tr_reg.sum()<400 or va.sum()<50:
            raise RuntimeError(f"F5_SMALL_FOLD:{month}:{tr.sum()}:{tr_reg.sum()}:{va.sum()}")
        ...
        for si,params in enumerate(SPECS):
            ...
            y_margin=(df.loc[tr_reg,"_home_f5"]-df.loc[tr_reg,"_away_f5"]).astype(float).to_numpy()
    ...
    home_comp=finite&(sp_adv>=0.50)&(ops_adv>=0.08)&(win_adv>=0.05)
    away_comp=finite&(sp_adv<=-0.50)&(ops_adv<=-0.08)&(win_adv<=-0.05)
    comp_mask=home_comp|away_comp
    comp_pred=np.where(home_comp,1,0)
    s=summarize(oof,comp_mask,comp_pred,{
      "architecture":"legacy_f5_composite",
    ...
    # F5 run-margin regression: a materially different architecture from the first pass.
    for mt in MARGIN_THRESHOLDS:
        mask=np.isfinite(pm)&(np.abs(pm)>=mt)
        pred=(pm>0).astype(int)
        s=summarize(oof,mask,pred,{
          "architecture":"catboost_f5_margin_regression",
          "mode":"symmetric","probability_threshold":None,
    ...
    if not sample_eligible:raise RuntimeError("F5_NO_SAMPLE_ELIGIBLE_CANDIDATE")
    ...
    result={
      "contract":"MLB_F5_ML_REVISIT_V1_RESULT/1.0.0",
      ...
      "folds":fold_meta,
      "architecture_family":"pregame_catboost_classifier_margin_f5_moneyline_v1",
    ...
    freeze={
      "contract":"MLB_F5_ML_REVISIT_V1_FROZEN_CANDIDATE/1.0.0",
      "protocol":PROTOCOL,"research_only":True,
      ...
      "reason":"Historical rolling F5 gate met before forward window." if target else "75% F5 revisit gate not met; no forward candidate activated.",
