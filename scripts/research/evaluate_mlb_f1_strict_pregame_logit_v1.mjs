...
const contractPath=new URL('../../contracts/MLB_F1_STRICT_PREGAME_LOGIT_V1.json',import.meta.url)
...
function standardizer(rows){
  const k=contract.features.length,means=Array(k).fill(0),sds=Array(k).fill(0)
...
const vector=(r,s)=>[1,...r.slice(8,18).map((v,j)=>(v-s.means[j])/s.sds[j])]
export function fit(rows){
  ...
  const s=standardizer(train),p=contract.features.length+1,lambda=contract.fitting.l2_lambda
  const home=train.filter(r=>r[3]>r[4]).length
  let beta=Array(p).fill(0)
  beta[0]=Math.log((home+1)/(train.length-home+1))
  let iterations=0
  for(;iterations<contract.fitting.max_iterations;iterations++){
    ...
    let maxD=0
    for(let j=0;j<p;j++){const next=clip(beta[j]+delta[j],contract.fitting.coefficient_clip);maxD=Math.max(maxD,Math.abs(next-beta[j]));beta[j]=next}
...
export function evaluate(input){
  ...
  for(const r of rows){
    assert(r.length===18,'SOURCE_SCHEMA')
    ...
    assert(typeof r[5]==='string'&&r[5]<r[2],'STRICT_PRIOR_CUTOFF')
    assert(r[6]==='xyear_raw_v1'&&r[7]==='A_XYEAR_RAW','FEATURE_ADMISSION')
    for(const v of r.slice(8,18))assert(Number.isFinite(v),'FEATURE_VALUE')
  ...
  return {
    ...
    state:pass?'DEVELOPMENT_GATE_PASS_EXTERNAL_NOT_OPENED':'DEVELOPMENT_GATE_FAILED_NO_EXTERNAL',confidence_threshold:contract.selection.confidence_threshold,
    external_opened:false,research_only:true,provider_calls_made:0,odds_api_historical_credits_consumed:0,official_picks_writes:0,apostar_activation:false,production_promotion:false,tracker_modified:false,fits,predictions
  }
}
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
  assert(process.argv[2]&&process.argv[3],'Usage: node evaluate_mlb_f1_strict_pregame_logit_v1.mjs input.json output.json')
