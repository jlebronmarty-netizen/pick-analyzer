...
def fetch_rows(token:str)->list[dict[str,Any]]:
    ...
    while True:
        ...
        if not resp.ok:
            raise RuntimeError(f"F7_3WAY_EXPORT_HTTP_{resp.status_code}:{resp.text[:800]}")
        p=resp.json()
        checks=[
    ...
    if expected is None or len(rows)!=expected:
        raise RuntimeError(f"F7_3WAY_EXPORT_COUNT:{len(rows)}:{expected}")
    return rows
...
def build_frame(rows:list[dict[str,Any]])->pd.DataFrame:
    ...
    for r in rows:
        ...
        if "actual_winner" in p or any(k.startswith(BLOCK_PREFIXES) for k in p):
            raise RuntimeError("F7_3WAY_POSTGAME_KEY_IN_PAYLOAD")
        y=str(r["y_class"])
        if y not in LABEL_TO_INT: raise RuntimeError(f"F7_3WAY_BAD_LABEL:{y}")
        d=p
    ...
    if (df["_game_date"]>MAX_HIST_DATE).any(): raise RuntimeError("F7_3WAY_HIST_CUTOFF_BREACH")
    if (df["_game_date"]==pd.Timestamp("2026-09-19")).any(): raise RuntimeError("F7_3WAY_QUARANTINE_BREACH")
    if (df["_game_date"]>=FORWARD_MIN_DATE).any(): raise RuntimeError("F7_3WAY_FORWARD_BREACH")
    if not (df["_development_class"]=="HISTORICAL_SEEN_DEVELOPMENT").all():
        raise RuntimeError("F7_3WAY_CLASS_BREACH")
    if not df["_research_only"].fillna(False).astype(bool).all():
        raise RuntimeError("F7_3WAY_RESEARCH_BREACH")
    cutoff=pd.to_datetime(df.get("feature_cutoff_date"),errors="coerce")
    if cutoff.notna().any() and (cutoff>=df["_game_date"]).any():
        raise RuntimeError("F7_3WAY_STRICT_PRIOR_BREACH")
...
def main():
    ...
    for month,start_s,end_s in FOLDS:
        ...
        if tr.sum()<400 or va.sum()<50:
            raise RuntimeError(f"F7_3WAY_SMALL_FOLD:{month}:{tr.sum()}:{va.sum()}")
        xv=x.loc[va]
        ...
        for si,params in enumerate(SPECS):
            ...
            rm=reg(params,si)
            margin=(df.loc[tr,"_home_f7"]-df.loc[tr,"_away_f7"]).astype(float).to_numpy()
    ...
    sp_adv=asp-hsp
    win_adv=hwp-awp
    home=np.isfinite(sp_adv)&np.isfinite(win_adv)&(sp_adv>=2.0)&(win_adv>=0.20)
    away=np.isfinite(sp_adv)&np.isfinite(win_adv)&(sp_adv<=-2.0)&(win_adv<=-0.20)
    legacy_mask=home|away
    legacy_pred=np.where(home,2,0)
    s=summarize(oof,legacy_mask,legacy_pred,{
      "architecture":"legacy_f7_3way_side_rule","mode":"side_only",
    ...
    if not sample:raise RuntimeError("F7_3WAY_NO_SAMPLE_ELIGIBLE_CANDIDATE")
    ...
    result={
      "contract":"MLB_F7_3WAY_REVISIT_V1_RESULT/1.0.0",
      ...
      "architecture_family":"pregame_catboost_multiclass_margin_f7_3way_v1",
    ...
    freeze={
      "contract":"MLB_F7_3WAY_REVISIT_V1_FROZEN_CANDIDATE/1.0.0",
      "protocol":PROTOCOL,"research_only":True,
