#!/usr/bin/env python3
"""Research-only MLB Totals V28 direct-Supabase ExtraTrees evaluation.
...
SEED = 20260917
TABLE = "mlb_totals_v28_full_individual_ml_dataset_2025_v1"
REGISTRY = "mlb_totals_formula_search_v1"
CANDIDATE_NAME = "totals_v28_individual_handedness_extra_trees_v1"
TARGET = "close_over_label"
EDGE_URL = "https://ynuocvexviorgdjrfthw.supabase.co/functions/v1/mlb-totals-v28-github-export-temp"
OUT = Path("artifacts/research/mlb_totals_v28_extra_trees_result.json")
...
def base_result(status: str) -> dict[str, Any]:
    return {
        "contract": "MLB_TOTALS_V28_EXTRA_TREES_RESEARCH/1.0.0",
        "research_only": True,
...
def fetch_range(oidc_token: str, range_name: str) -> list[dict[str, Any]]:
    ...
    if not r.ok:
        raise RuntimeError(f"V28_EDGE_HTTP_{r.status_code}:{r.text[:1000]}")
    payload = r.json()
    checks = [
        (payload.get("contract") == "MLB_TOTALS_V28_GITHUB_EXPORT/1.0.0", "contract"),
    ...
    if bad:
        raise RuntimeError("V28_EDGE_CONTRACT_INVALID:" + ",".join(bad))
    return payload["rows"]
...
def main() -> None:
    ...
    if len(apr_jun) != 1150:
        raise RuntimeError(f"V28_APR_JUN_COUNT_MISMATCH:{len(apr_jun)}")
    ...
    if len(train) != 769 or len(june) != 381:
        raise RuntimeError(f"V28_SPLIT_COUNT_MISMATCH:{len(train)}:{len(june)}")
    ...
    if selected is not None:
        # First and only read of Jul-Aug for this frozen V28 family.
        jul_aug = fetch_range(oidc_token, "jul_aug")
        if len(jul_aug) != 741:
            raise RuntimeError(f"V28_JULAUG_COUNT_MISMATCH:{len(jul_aug)}")
