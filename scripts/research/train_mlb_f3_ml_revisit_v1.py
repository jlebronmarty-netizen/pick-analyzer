...
SEED=20260919
EDGE_URL="https://ynuocvexviorgdjrfthw.supabase.co/functions/v1/mlb-f3-ml-revisit-github-export-temp"
EDGE_CONTRACT="MLB_F3_ML_REVISIT_GITHUB_EXPORT/1.0.0"
...
OUT=Path("artifacts/research/mlb_f3_ml_revisit_v1_result.json")
FREEZE=Path("artifacts/research/mlb_f3_ml_revisit_v1_frozen_candidate.json")

TARGET_ACC=0.75
MIN_N=60
MIN_MONTHS=5
MIN_WORST=0.65
LEGACY_THRESHOLD=0.268037684706378
...
def fetch_rows(token:str)->list[dict[str,Any]]:
    ...
    while True:
        ...
        if not resp.ok:
            raise RuntimeError(f"F3_EXPORT_HTTP_{resp.status_code}:{resp.text[:800]}")
        p=resp.json()
        checks=[
        ...
        if bad: raise RuntimeError("F3_EXPORT_INVALID:"+",".join(bad))
        if expected is None: expected=int(p["totalRows"])
        if int(p["totalRows"])!=expected: raise RuntimeError("F3_EXPORT_TOTAL_CHANGED")
        batch=p.get("rows") or []
        rows.extend(batch);offset+=len(batch)
        if not batch or offset>=expected: break
    if expected is None or len(rows)!=expected:
        raise RuntimeError(f"F3_EXPORT_COUNT:{len(rows)}:{expected}")
    return rows
...
def build_frame(rows:list[dict[str,Any]])->tuple[pd.DataFrame,int]:
    ...
    if (df["_game_date"]>MAX_HIST_DATE).any(): raise RuntimeError("F3_HIST_CUTOFF_BREACH")
    if (df["_game_date"]==pd.Timestamp("2026-09-19")).any(): raise RuntimeError("F3_QUARANTINE_BREACH")
    if (df["_game_date"]>=FORWARD_MIN_DATE).any(): raise RuntimeError("F3_FORWARD_BREACH")
    if not (df["_development_class"]=="HISTORICAL_SEEN_DEVELOPMENT").all():
        raise RuntimeError("F3_CLASS_BREACH")
    if not df["_research_only"].fillna(False).astype(bool).all():
        raise RuntimeError("F3_RESEARCH_BREACH")
    cutoff=pd.to_datetime(df.get("feature_cutoff_date"),errors="coerce")
    if cutoff.notna().any() and (cutoff>=df["_game_date"]).any():
        raise RuntimeError("F3_STRICT_PRIOR_BREACH")
...
def main():
    ...
    if not token: raise RuntimeError("F3_REVISIT_OIDC_MISSING")
    ...
    for month,start_s,end_s in FOLDS:
        ...
        for si,params in enumerate(SPECS):
            ...
            y_margin=(df.loc[tr_reg,"_home_f3"]-df.loc[tr_reg,"_away_f3"]).astype(float).to_numpy()
    ...
    # F3 run-margin regression: a materially different architecture from the first pass.
    for mt in MARGIN_THRESHOLDS:
        mask=np.isfinite(pm)&(np.abs(pm)>=mt)
        pred=(pm>0).astype(int)
        s=summarize(oof,mask,pred,{
          "architecture":"catboost_f3_margin_regression",
    ...
    if not sample_eligible:raise RuntimeError("F3_NO_SAMPLE_ELIGIBLE_CANDIDATE")
    ...
    result={
      "contract":"MLB_F3_ML_REVISIT_V1_RESULT/1.0.0",
      ...
      "folds":fold_meta,
      "architecture_family":"pregame_catboost_classifier_margin_f3_moneyline_v1",
    ...
    freeze={
      "contract":"MLB_F3_ML_REVISIT_V1_FROZEN_CANDIDATE/1.0.0",
      "protocol":PROTOCOL,"research_only":True,
      ...
      "reason":"Historical rolling F3 gate met before forward window." if target else "75% F3 revisit gate not met; no forward candidate activated.",
