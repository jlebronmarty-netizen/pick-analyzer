...
check('script exists and is dry-run by default', script.includes("const dryRun = !execute || args.has('--dry-run')"))
...
check('source contract reads existing raw only', script.includes("from('pick2_raw_mlb_statcast_pitches')") && !/statsapi\.mlb\.com|api\.sportsdata\.io|api\.the-odds-api\.com|api\.balldontlie/i.test(script))
...
check('conflict contract encoded', script.includes('pitcherConflicts') && script.includes('batterConflicts') && script.includes('R5B_PREWRITE_CONFLICTS_DETECTED'))
check('execution body implemented', script.includes('executeNativeGameBackfill') && script.includes('executeNativePlayerBackfill') && script.includes('executeRawBackfill'))
