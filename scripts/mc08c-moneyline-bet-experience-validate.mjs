...
check('no historical leakage path is introduced', !/history|historical|settled/i.test(home.match(/function buildMoneylineBetContract[\s\S]*?function buildSmartParlayContract/)?.[0] ?? home.match(/function buildMoneylineBetContract[\s\S]*?function dailyRecommendation/)?.[0] ?? ''))
...
check('empty waiting review-only states are supported', home.includes('No Eligible Moneyline Bet') && home.includes('Waiting for Fresh Price') && (home.includes('Review-Only Moneyline Candidate') || home.includes('Best Moneyline Candidate')))
