...
check('current mission is MC-00 or later Mission Control mission', ['MC-00', 'MC-01', 'MC-02', 'MC-08H', 'OR-01H', 'OR-02', 'OR-02A', 'PRODUCTION-PILOT-WEEK'].includes(status.currentMission?.id))
check('next mission is deterministic', status.nextMission === null || ['MC-01', 'MC-02', 'MC-08', 'OR-01D', 'OR-01H', 'OR-02', 'OR-02A', 'PRODUCTION-PILOT-WEEK'].includes(status.nextMission?.id))
check('only MC-08 bounded product package or scheduler decision gates may be active/ready in status', status.missionCounts?.active === 0 || (status.missionCounts?.active === 1 && status.nextMission?.id === 'MC-08' && status.nextMission?.state === 'ACTIVE') || (status.nextMission?.id === 'OR-01D' && status.nextMission?.state === 'EXTERNAL_WAIT') || status.currentMission?.id === 'OR-01H' || status.currentMission?.id === 'OR-02' || status.currentMission?.id === 'OR-02A' || status.currentMission?.id === 'PRODUCTION-PILOT-WEEK' || (status.nextMission?.id === 'PRODUCTION-PILOT-WEEK' && status.nextMission?.state === 'READY'))
check('OR scheduler status overlays have mission-specific metadata', service.includes('Operations Architecture') && service.includes('human_vercel_plan_and_cron_settings_check_required') && service.includes("id === 'OR-02'") && service.includes('three_consecutive_vercel_primary_executions_required') && service.includes("id === 'OR-02A'") && service.includes('market_freshness_and_product_readiness_recovery_required'))
...
check('queue doc keeps future gated missions inactive', queueDoc.includes('MC-03 remains PLANNED') && (queueDoc.includes('MC-08 | Daily Betting Product Completion | PRODUCT_EXPERIENCE | P2 | ACTIVE') || queueDoc.includes('MC-08 | Daily Betting Product Completion | PRODUCT_EXPERIENCE | P2 | PRODUCTION_CERTIFIED') || queueDoc.includes('MC-08 | Daily Betting Product Completion | PRODUCT_EXPERIENCE | P2 | BLOCKED')))
