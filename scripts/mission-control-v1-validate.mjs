...
check('current mission is MC-00', status.currentMission?.id === 'MC-00')
check('next mission is MC-01', status.nextMission?.id === 'MC-01')
check('no future mission marked active in status', status.missionCounts?.active === 0)
...
check('MLB is certified only sport', status.sportReadiness.MLB === 'CERTIFIED' && !Object.entries(status.sportReadiness).some(([sport, state]) => sport !== 'MLB' && state === 'CERTIFIED'))
...
check('queue doc keeps future missions inactive', queueDoc.includes('No queued mission is ACTIVE'))
...
check('project status updated for Mission Control', projectStatus.includes('Mission Control V1') && projectStatus.includes('next eligible mission is MC-01'))
