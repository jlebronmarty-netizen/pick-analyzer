...
check('Mission Control selected MC-08', status.nextMission?.id === 'MC-08' && queue.includes('| MC-08 | Daily Betting Product Completion'))
...
check('Decision Core Morning Brief is first visible section', home.indexOf('data-mc08a-morning-brief="true"') < home.indexOf('title="Rent Play"'))
...
check('Rent Play remains and is largest/full-width before Moneyline', home.indexOf('title="Rent Play"') < home.indexOf('title="Moneyline Bet"') && home.includes('No Rent Play Today'))
check('Moneyline Bet remains', home.includes('title="Moneyline Bet"') && home.includes("item.market.toLowerCase().includes('moneyline')"))
check('Smart Parlay replaces primary Parlay Builder label while preserving compatibility phrase', home.includes('Smart Parlay') && home.includes('Parlay Builder math') && home.includes('type="checkbox"'))
check('Smart Parlay live metrics remain client-side', home.includes('const probability = selected.reduce') && home.includes('const confidence = selected.length') && home.includes('const ev = selected.length'))
...
check('best opportunity no-bet compatibility retained', home.includes('No Qualified Opportunity Today') && home.includes("!/avoid|do not act/i.test(item.reason)"))
