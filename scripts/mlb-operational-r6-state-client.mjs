...
export function createDurableRuntimeClient({url,key,packageSha,deadline=Infinity}) {
  ...
  return {
    ...
    async resumeMarket({runId,originalPackageSha,expectedDigest,marketReadbackDigest}) {
      ensure(!lease,'ALREADY_HELD')
      const result=await call({op:'resumeMarket',holder,runId,packageSha:originalPackageSha,executorPackageSha:packageSha,expectedDigest,marketReadbackDigest})
      if(result.status==='ACQUIRED'){lease=result.lease;run=result.run;missionOddsCalls=result.missionOddsCalls}
      return result
    },
    ...
    async resumeMarketCheckpoint({runId,originalPackageSha,expectedDigest,marketReadbackDigest}) {
      ensure(!lease,'ALREADY_HELD')
      const result=await call({op:'resumeMarketCheckpoint',holder,runId,packageSha:originalPackageSha,executorPackageSha:packageSha,expectedDigest,marketReadbackDigest})
      if(result.status==='ACQUIRED'){lease=result.lease;run=result.run;missionOddsCalls=result.missionOddsCalls}
      return result
    },
