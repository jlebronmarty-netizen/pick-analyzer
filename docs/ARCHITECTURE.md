...
`src/services/nhl-historical-foundation-v2.service.ts` and `/api/data-foundation/nhl` provide a read-only NHL foundation audit for cross-year season coverage. The audit reports schedule, result, stats, period-score, goalie/starter, injury, odds, mapping and prediction readiness while explicitly preventing NHL pick generation when stored data or certified engine readiness is unavailable.
...
- AI learning lifecycle: `ai-learning-lifecycle.service.ts` powers `/api/ai-operations/lifecycle` and `/ai-operations`. It reads persisted schedule, odds, prediction, settlement, replay/projection, feature snapshot, model-weight, calibration, provider-budget and scheduler evidence to validate the autonomous daily AI chain. The service creates only a derived queue/evidence contract and does not execute Learning Brain updates, replay generation, prediction generation, provider refreshes or Official Pick policy changes. Accepted samples require deterministic labels and point-in-time feature evidence; trained/applied states require persisted model-weight evidence.
...
- Daily settlement closure: Settlement V2 can repair policy-skipped rows that were misclassified as test/fixture data when exact persisted final scores and pregame timestamps exist. Trial, scrambled, fixture, quarantine, synthetic, post-start, duplicate, settled, void and closed protections remain intact.
...
- Prediction cutoff enforcement: `prediction-cutoff-enforcement.service.ts` is the shared guard for pregame prediction eligibility. It classifies rows as `PREGAME`, `POST_START`, `POST_FINAL` or `INVALID_CUTOFF` from persisted prediction timestamps, cutoff timestamps, canonical event start and final-observed timestamps when available. Shared prediction-history persistence rejects production-eligible post-cutoff rows, MLB prospective preview rejects direct post-cutoff candidates before snapshot/prediction upsert, and settlement, learning, performance and trace diagnostics exclude contaminated rows from production use. Existing excluded rows carry persisted diagnostic metadata in `prediction_history.settlement_details.cutoff_enforcement_v1`; the persistence scope is classification metadata only and does not rewrite probabilities, timestamps, outcomes, settlement or learning evidence.
...
- Model-only intelligence: `model-only-intelligence.service.ts` reads stored `prediction_history` and `universal_projection_history` to expose Most Likely model rankings, probable moneyline, informational parlays and pitcher-outs shadows without requiring odds or becoming Official Picks. When Today has no future pregame slate, the informational fallback may use the next stored pregame slate; Best Value and Official Pick gates remain odds/policy controlled.
- Probability Picks: `probability-picks.service.ts` exposes projection-only probability rankings and correlation-aware parlays through `/api/probability-picks*` and `/probability-picks`. It reads only model/projection columns from stored prediction rows plus the existing MLB Pitcher Projection Engine thresholds, omits sportsbook/price/edge fields, performs no provider calls or mutations and does not become Current Board, Most Likely, Best Value, Official Picks, EV, Kelly or Portfolio Intelligence.
- Probability Picks V2: the same service and routes add backward-compatible metadata for `version: probability_picks_v2`, sport eligibility details, top signals, freshness summaries, filter/sort metadata, briefing context, pick explanations and parlay presentation blockers. V2 keeps `mode: probability_picks_v1` for compatibility and does not change probability, confidence, quality, thresholds, score formulas, parlay thresholds, correlation math, models, scheduler, Learning Brain or persistence.
- Performance Product Mode: `ai-performance-center.service.ts` and `performance-scope-v2.service.ts` default user-facing performance to production-evaluable Win/Loss/Push rows. Legacy, Ignored, Historical, Replay, Shadow, Cancelled, Voided, test-like and unresolved rows are audit-only. Empty settled samples return nullable accuracy and render as `N/A`. Production time-window grouping prefers canonical `sport_events.start_time` over prediction `commence_time` so event-linked same-day predictions are counted consistently.
- Product live-state read models: default product pages should prefer compact read models over heavy diagnostics. `/api/performance/history` uses `performance-scope-v2.service.ts` compact `historyRows` with bounded pagination so the Performance page does not load the full AI Performance Center graph or large feature snapshots for table rendering. Dashboard Today game cards use persisted per-event settlement state from `prediction_history`, and the User Mode pipeline summary renders the existing Today pipeline plus scheduler/freshness metadata instead of recomputing contradictory UI-only states.
- Recommendation pipeline trace: `recommendation-pipeline-trace.service.ts` powers `/api/recommendation-pipeline/trace`, a read-only Today/Yesterday evidence report for schedule, odds, predictions, Current Board, Model Only, recommendation buckets, settlement labels and learning persistence. It also exposes per-game lifecycle diagnostics, miss reasons, coverage percentages and no-post-start/no-post-final prediction checks. It performs no provider calls, no prediction generation, no learning execution and no mutations.
- Pregame execution recovery: `mlb-operating-date-resolution.service.ts` selects the earliest pregame-actionable MLB slate for provider-backed odds, prediction and Current Board refresh work. A date is actionable only when at least one game remains before the ten-minute prediction cutoff. When the local slate is already past cutoff, refresh actions pivot to the next safe slate, while status/results recovery keeps its bounded recovery behavior. `prepare_next_slate` can prewarm the next calendar date through the existing SportsDataIO prospective-preview service when no future slate is stored yet, using the same provider budget, action lock, deterministic upsert and cutoff-enforcement path.
- Pregame scheduler coverage: `pregame-scheduler-coverage.service.ts` reads persisted MLB events, odds snapshots, prediction history, cutoff classifications, operating-day lifecycle events and sync jobs to expose scheduler timing, next execution, average duration, per-game prediction lead time before cutoff, rejection reasons, retry policy and duplicate/idempotency evidence. It also exposes a forward-looking `nextPregameSlate` with pregame coverage, lead time, board-ready games, pending execution count and cutoff-protected games. It is consumed by Recommendation Pipeline Trace, Performance History, Dashboard Today and AI Operations. It does not execute schedulers, call providers, rebuild Current Board, generate predictions, settle rows, run learning, start replay or modify historical feature-store data.
...
- Local Retrosheet feature backfill worker: `scripts/retrosheet-feature-backfill.mjs` runs Phase 2A generation outside Vercel serverless limits using `.env.local`, the existing `supabaseAdmin` client and the Phase 2A service. `scripts/local-ts-loader.mjs` provides local Node compatibility for repo aliases and `server-only`. The worker supports dry-run, validate, full, resume, idempotency and single-game modes, writes checkpoints in `historical_import_checkpoints`, and never changes production prediction, odds, settlement, Current Board, Official Pick or Learning Brain state.
