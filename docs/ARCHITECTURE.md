...
- AI learning lifecycle: `ai-learning-lifecycle.service.ts` powers `/api/ai-operations/lifecycle` and `/ai-operations`. It reads persisted schedule, odds, prediction, settlement, replay/projection, feature snapshot, model-weight, calibration, provider-budget and scheduler evidence to validate the autonomous daily AI chain. The service creates only a derived read-only learning queue and does not execute Learning Brain updates, replay generation, settlement writes, prediction generation, provider refreshes or Official Pick policy changes.
