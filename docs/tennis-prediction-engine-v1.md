...
Tennis Prediction Engine V1 creates a tennis-specific prediction orchestration layer on the Shared Sport Prediction Engine SDK without ML training, live provider calls, persistence or production betting claims.

## Scope

- Service: `src/services/tennis-prediction-engine.service.ts`
- APIs:
  - `GET /api/tennis/predictions`
  - `GET /api/tennis/predictions/health`
  - `GET /api/tennis/predictions/validation`
- Dashboard: `TennisPredictionEnginePanel` in the multi-sport dashboard section.

## Markets

The deterministic V1 preview covers:

- match winner through the shared moneyline contract
- match total through the shared total contract

The engine intentionally does not implement player props, live betting, set props, point props or ML training.
...
- Feature Store Core and Tennis Feature Store Integration V1 for feature snapshots.
...
Real-data validation and historical calibration should wait for normalized tennis event, odds, result, player-form, surface, ranking and injury coverage.
