...
- For model-only MLB intelligence, use stored `prediction_history` and `universal_projection_history` only. Label outputs as informational/model-only/shadow and never compute EV, Kelly, stake or Official Pick status without market odds and policy eligibility.
...
- For Retrosheet Phase 2A historical feature-store work, run `GET /api/mlb/historical-intelligence/retrosheet/features?view=contract`, `VALIDATE_ONLY`, `DRY_RUN` and representative `SINGLE_GAME_PREVIEW` first. `FULL_SEASON_IMPORT` writes historical feature snapshots plus job/checkpoint rows and may proceed only when the protected production write is explicitly approved and passes approval review. Never treat DRY_RUN readiness as permission to import.
