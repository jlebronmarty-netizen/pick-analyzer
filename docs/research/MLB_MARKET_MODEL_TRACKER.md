...
| **Pitcher Outs** | `pitcher_outs_under_18p5_p90_v1` — historical frozen UNDER 18.5 rule | Historical event-accuracy evidence only; live input parity unresolved | Frozen evidence: 215/226 = **95.13%**; exact-MLBAM reconstruction selects 254/243 instead of the frozen 226/215 universe | **NOT RUNTIME-CERTIFIED** — no recommendation is permitted even if a real 18.5 quote exists | Frozen n=226; exact reconstruction n=254 | **YES historically; NO runtime certification** | `RUNTIME_PARITY_NOT_CERTIFIED` | Fail closed until `PITCHER_OUTS_INPUT_LINEAGE_NOT_EXACTLY_RECONCILED` is resolved without retuning. |
