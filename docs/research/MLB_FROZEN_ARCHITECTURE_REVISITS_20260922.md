...
Follow-up correction: local BALLDONTLIE historical opening access was subsequently verified HTTP 200. A separate bounded three-call capability probe returned an empty historical period catalog for the sampled exact BDL game. Model-evaluation provider calls remain zero; session provider calls are now three BDL calls and zero Odds API calls/credits. See `MLB_BDL_ACCESS_AND_LINE_BLOCKER_20260922.md` and its sanitized ledger. Missing local credentials are not the active blocker.
