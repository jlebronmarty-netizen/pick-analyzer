...
The Odds API Maximum Utilization, Historical Odds Acquisition And Multi-Sport Prediction Enablement V1 is in controlled execution from commit `19650ee34f680558e9edab4109c88b556828e0d5`. Checkpoint 1 adds bounded The Odds API catalog, quota, capability and coverage API contracts plus Data Coverage navigation. Checkpoint 2 adds bounded current core odds acquisition for mapped active sports, storing provider-native event mappings and h2h/spread/total snapshots with sanitized credit evidence. Checkpoint 3 proves current player-prop discovery as truthful empty coverage for the tested near-term MLB/NFL event-market set under the hard call cap. Checkpoint 4 proves historical h2h capability for MLB, NFL, NHL, NBA and UFC with 250 credits consumed and no row imports; broad historical import now requires narrower priority windows to preserve the 2,000-credit reserve. Checkpoint 5 imports a narrow MLB 2026 historical h2h/spread/total sample with pre/post-start timestamp classification. Checkpoint 6 records non-MLB score endpoint behavior without overriding stronger result sources. Checkpoint 7 derives market-history and closing-candidate evidence from stored snapshots without duplicating raw data. Feature rebuilds, prediction activation and scheduler continuity remain future checkpoint work.
