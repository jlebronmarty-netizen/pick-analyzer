...
MLB-04D-D3S-R1 Immutable Opportunity Evidence Repair is locally implemented as `MLB_04D_D3S_R1_IMMUTABLE_OPPORTUNITY_EVIDENCE_REPAIR_CERTIFIED`. It separates mutable prospective current-board rows from frozen research evidence by preparing an additive `mlb_forward_opportunity_evidence` table and a default-off D3W append/reuse hook guarded by `MLB_FORWARD_OPPORTUNITY_EVIDENCE_AUTHORIZED=true`. No old rows are backfilled, no production evidence rows are written during certification, no cron is activated and no ledger row or Observation #4 is created. Next: publish D3S-R1, apply/verify the additive storage migration only under explicit authorization, then run a bounded one-event snapshot + immutable opportunity + ledger canary.
