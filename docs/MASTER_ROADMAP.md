...
Current MLB roadmap sequence: prepare `MLB_DATA_02A_INDIVIDUAL_PICK_MODEL_DATASET_PREPARATION`; train and validate market-specific individual-pick probability models only after a separate training authorization; add no-vig/value scoring; build Value Board and pick-detail explanations; certify selective Official Picks; then add optional user-selected parlay analysis only after individual-leg value and correlation contracts are certified. `SPORTSDATAIO_MLB_REQUIRED_BY_PICK2 = NO`, and MLB SportsDataIO credential repair remains out of scope for Pick 2 MLB.
