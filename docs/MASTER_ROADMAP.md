...
Current MLB roadmap sequence: separately authorize `MLB_DATA_02O_R1_NATIVE_VALUE_SCHEMA_REPAIR_PREP`; prepare a non-applied forward migration for immutable native value-evaluation storage rooted in persisted prediction IDs and `pick2_mlb_market_price_observations`; then separately authorize migration application/readback and retry value persistence from the certified 02N artifact. Value Board exposure, pick-detail explanations and selective Official Pick policy remain downstream. Optional user-selected parlay analysis remains after individual-leg value and correlation contracts are certified. `SPORTSDATAIO_MLB_REQUIRED_BY_PICK2 = NO`, and MLB SportsDataIO credential repair remains out of scope for Pick 2 MLB.
