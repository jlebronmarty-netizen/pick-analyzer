...
Current MLB roadmap sequence: separately authorize `MLB_DATA_02N_CURRENT_MONEYLINE_VALUE_EVALUATION_PREP`; prepare no-vig/implied probability and value-scoring contracts from persisted current market observations; then separately authorize any value persistence, Value Board exposure, pick-detail explanations and selective Official Picks. Optional user-selected parlay analysis remains after individual-leg value and correlation contracts are certified. `SPORTSDATAIO_MLB_REQUIRED_BY_PICK2 = NO`, and MLB SportsDataIO credential repair remains out of scope for Pick 2 MLB.
