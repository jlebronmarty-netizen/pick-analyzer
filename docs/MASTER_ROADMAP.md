...
MLB-01 Context Lineage is locally implemented as `MLB_01_CONTEXT_LINEAGE_PARTIAL_CERTIFICATION`. It adds a shadow-only `mlb_context_snapshots` contract plus a read-only context-lineage API/script for current and forward MLB events. Approved sources are MLB Official for schedule/probable starter/venue evidence, The Odds API for odds and stored canonical tables for lineups, bullpen and injuries. SportsDataIO remains rollback-only and excluded. Weather and richer injury/confirmed-lineup context remain separate provider-certification blockers; prediction formulas, Official Picks, settlement, learning and calibration remain unchanged. Next: publish/deploy the bounded MLB-01 lineage package, then run a protected current/forward context probe before considering any model-input consumer wiring.
