...
Current MLB roadmap sequence: separately authorize `MLB_DATA_02L_CURRENT_MONEYLINE_MARKET_PERSISTENCE_SCHEMA_PREP`; add no-vig/value scoring only after market-price observations are persisted under a certified native immutable schema; build Value Board and pick-detail explanations; certify selective Official Picks; then add optional user-selected parlay analysis only after individual-leg value and correlation contracts are certified. `SPORTSDATAIO_MLB_REQUIRED_BY_PICK2 = NO`, and MLB SportsDataIO credential repair remains out of scope for Pick 2 MLB.
