...
Current MLB roadmap sequence: separately authorize `MLB_DATA_02M_R3_FRESH_MARKET_SAMPLE_PERSISTENCE_EXECUTION` before any market DML; persist only the frozen 02M-R2 rows under the certified caps; add no-vig/value scoring only after market-price observations are persisted under the certified native immutable schema; build Value Board and pick-detail explanations; certify selective Official Picks; then add optional user-selected parlay analysis only after individual-leg value and correlation contracts are certified. `SPORTSDATAIO_MLB_REQUIRED_BY_PICK2 = NO`, and MLB SportsDataIO credential repair remains out of scope for Pick 2 MLB.
