...
PICK-2.0 RESET-04R1 is locally implemented as `PICK_2_RESET_04R1_STATCAST_SCHEMA_COMPATIBILITY_CERTIFIED`. It supersedes the pre-apply RESET-04 raw Statcast column contract after the real 2026 YTD source audit and updates the still-unapplied migration rather than adding follow-up ALTERs. The source pitch identity remains `game_pk + at_bat_number + pitch_number`, all 22 audited columns are accounted for, source MLBAM player/team evidence is separated from canonical Pick IDs, post-score progression is stored for label reconstruction only, and unsupported fields such as launch speed, launch angle, barrels, xwOBA, spin and plate location remain explicitly unsupported until a richer source is certified. Next: publish RESET-04R1, then resume RESET-05 migration apply/readback; do not import Statcast during RESET-05.
