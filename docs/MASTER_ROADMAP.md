...
Current MLB roadmap sequence: finish `MLB_DATA_01D_R1H_MANUAL_SCHEMA_READBACK_COMPLETION`; separately authorize `MLB_DATA_01D_R1I_PARTIAL_FEATURE_DML_RESUME_EXECUTION`; complete 01D feature persistence for individual pick modeling; prepare `MLB_DATA_02A_MARKET_SPECIFIC_MODEL_DATASET_PREPARATION`; train and validate market-specific individual-pick probability models; add no-vig/value scoring; build Value Board and pick-detail explanations; certify selective Official Picks; then add optional user-selected parlay analysis only after individual-leg value and correlation contracts are certified. `SPORTSDATAIO_MLB_REQUIRED_BY_PICK2 = NO`, and MLB SportsDataIO credential repair remains out of scope for Pick 2 MLB.
