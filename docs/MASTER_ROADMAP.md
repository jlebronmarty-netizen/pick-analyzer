...
Current MLB roadmap sequence: separately authorize `MLB_DATA_02O_CURRENT_MONEYLINE_VALUE_EVALUATION_PERSISTENCE`; persist immutable value-evaluation rows only if the 02N analytical contracts and zero-write readback remain valid; then separately prepare Value Board exposure, pick-detail explanations and selective Official Pick policy. Optional user-selected parlay analysis remains after individual-leg value and correlation contracts are certified. `SPORTSDATAIO_MLB_REQUIRED_BY_PICK2 = NO`, and MLB SportsDataIO credential repair remains out of scope for Pick 2 MLB.
