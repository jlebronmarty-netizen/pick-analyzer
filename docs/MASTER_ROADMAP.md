...
R4 resolves unattended schema access through a service-authenticated Supabase Edge Function using fixed read-only SQL and the existing shared schema checker. Production preflight passes 406 columns, 116 constraints, 84 indexes and all six snapshot domains. The existing tick fetches a fresh preflight before execution. Automation remains disabled pending a verified persistent executor host: the existing Git/filesystem/checkpoint contracts cannot run safely on independent ephemeral Vercel instances. No R4 providers, production DML or DDL; settlement automation remains disabled. See `docs/CERTIFICATION/MLB_DATA_02R_R4_UNATTENDED_PREFLIGHT_CERTIFICATION.json` and its audit. Required next gates are persistent-host verification, full unattended dry certification, activation and actual scheduled readback; final operational certification is not issued.
