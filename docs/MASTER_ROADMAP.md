...
MLB-01R2 Context Source Repair is locally implemented but production persistence is blocked as `MLB_01R2_CONTEXT_SOURCE_REPAIR_BLOCKED`. The code now skips unsafe rows independently, preserves deterministic insert-only context snapshot identity and uses bounded MLB Official live-feed batting-order evidence as projected lineup context. Production persistence selected 7 eligible MORNING rows and skipped 3 unsafe rows, but Supabase/PostgREST returned `PGRST205` for real `public.mlb_context_snapshots` row/column reads, so 0 snapshot rows were written. SportsDataIO remained 0 and prediction/model/recommendation/settlement/learning behavior was unchanged. Next: apply or refresh the existing `202608200001_mlb_context_snapshots_v1.sql` production schema contract, then rerun R2 persistence/idempotency proof.
