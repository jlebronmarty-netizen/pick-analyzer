...
MLB-01R2A Context Snapshot Event ID Type Repair is locally certified as `MLB_01_CONTEXT_SNAPSHOT_EVENT_ID_TYPE_REPAIR_CERTIFIED`. Manual production SQL proved `public.sport_events.id` is `text` and `public.mlb_context_snapshots` is not created; the unapplied migration now uses `event_id text references public.sport_events(id)` while preserving the UUID row surrogate, deterministic key, indexes, RLS and service-role policy. No provider calls or production DB mutations occurred. Next: manually apply the corrected migration, then publish R2/R2A and rerun context snapshot persistence/readback/idempotency proof.
