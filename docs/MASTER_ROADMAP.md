...
**MLB autonomous operational mission: authorized DDL certified; R2T remains in progress.** The exact snapshot-uniqueness proposal was applied after fresh preflight and rollback validation. Independent readback preserves all 118,064 daily rows, their full-row digests, FKs/PKs/checks and zero orphan references. Six native snapshot-unique indexes now support immutable revisions. The unpublished repair commit is being sanitized to exclude raw samples; private replay inputs remain external. The original 51 checks plus two rollback checks pass. Next: sanitized normal publication, then revision-aware reads, all-game real R2B/checkpoint integration and R3 before any live run; retain the standing live/repeatability/automation/settlement/Today sequence and budgets. Providers and row DML remain zero; exactly one authorized DDL migration has run. See the structural DDL certificate and mission audit in `docs/CERTIFICATION`.
