...
Vercel Build Memory Recovery V1 is locally complete. Build-memory root cause is classified as duplicated Supabase server dependency bundling plus webpack single-process pressure after Phase A had already reduced prerender routes to 6. `next.config.ts` now externalizes `@supabase/supabase-js` for server builds and re-enables the webpack build worker. Repeat clean build peak memory improved from the prior Phase B final 2847.6 MB to 2414.0 MB, a 15.23% reduction, while generated static pages remained 386 and app page routes remained 30. No product routes, API contracts, business rules, prediction logic, scheduler behavior, model weights, provider behavior or database schema changed. Vercel Standard-machine recovery remains to be proven by automatic deployment after push; no manual Vercel deployment was performed.

Evidence: `docs/VERCEL_BUILD_MEMORY_RECOVERY_V1.md`, `docs/vercel-build-memory-recovery-v1-summary.json`, `docs/build-memory-optimization-v1-phase2-repeat.json`, `docs/build-memory-optimization-v1-phase2-route-manifest.json`, `docs/build-memory-optimization-v1-phase2-import-pressure.json` and `scripts/vercel-build-memory-recovery-v1-validate.mjs`.
