...
R4 implements the fixed-query unattended Supabase Edge preflight with shared schema checks. The local server credential passes all production schema checks (406 columns, 116 constraints, 84 indexes and six snapshot domains). Production Vercel has a server credential configured but receives HTTP 401 from exact legacy/modern server-key authentication; secure credential alignment requires an authenticated Vercel management connection. The persistent R2 executor host is also unverified. Automation and automatic settlement remain disabled. No R4 sports providers, production DML or DDL. See `docs/CERTIFICATION/MLB_DATA_02R_R4_UNATTENDED_PREFLIGHT_CERTIFICATION.json`; final operational certification is not issued.
