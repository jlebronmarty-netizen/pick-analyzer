-- Repair the pitcher-game runtime materialization so refreshes read canonical
-- Statcast rather than the compatibility view that will point back to the MV.
...
drop materialized view if exists public.mlb_statcast_pitcher_game_logs_mv;

create materialized view public.mlb_statcast_pitcher_game_logs_mv as
select
  game_year as season,
  game_pk,
  min(game_date) as game_date,
...
create unique index mlb_statcast_pitcher_game_logs_mv_uidx
  on public.mlb_statcast_pitcher_game_logs_mv (season, game_pk, pitcher);
create index mlb_statcast_pitcher_game_logs_mv_pitcher_date_idx
  on public.mlb_statcast_pitcher_game_logs_mv (season, pitcher, game_date desc, game_pk);
create index mlb_statcast_pitcher_game_logs_mv_workload_idx
...
create or replace view public.mlb_statcast_pitcher_game_logs
with (security_invoker = true) as
select * from public.mlb_statcast_pitcher_game_logs_mv;
