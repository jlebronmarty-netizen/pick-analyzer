...
const CONTRACT="MLB_F1_NRFI_REVISIT_GITHUB_EXPORT/1.0.0";
const AUDIENCE="supabase-f1-nrfi-revisit";
const REPOSITORY="jlebronmarty-netizen/pick-analyzer";
const ALLOWED_REF="refs/heads/research/f1-nrfi-revisit-20260919";
...
Deno.serve(async(req:Request)=>{
  ...
  const {data,error,count}=await db
    .from("mlb_f1_nrfi_revisit_base_v1")
    .select("season,game_pk,game_date,home_f1,away_f1,y_nrfi,home_sp_mlbam_id,away_sp_mlbam_id,payload,outcome_lineage,development_class,research_only",{count:"exact"})
  ...
  return json({
    contract:CONTRACT,researchOnly:true,sourceTable:"public.mlb_f1_nrfi_revisit_base_v1",
    historicalMaxGameDate:"2026-09-18",returned:data?.length??0,totalRows:count??null,
    offset,limit,rows:data??[],forwardIncluded:false,
    oddsApiHistoricalCreditsConsumed:0,officialPicksWrites:0,
    apostarActivation:false,productionPromotion:false
  });
});