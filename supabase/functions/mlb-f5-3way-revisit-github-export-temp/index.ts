...
const CONTRACT="MLB_F5_3WAY_REVISIT_GITHUB_EXPORT/1.0.0";
const AUDIENCE="supabase-f5-3way-revisit";
const REPOSITORY="jlebronmarty-netizen/pick-analyzer";
const ALLOWED_REF="refs/heads/research/f5-3way-revisit-20260919";
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const expectedWorkflowRef=REPOSITORY+"/.github/workflows/mlb-f5-3way-revisit-v1.yml@"+ALLOWED_REF;
  ...
  const {data,error,count}=await db.from("mlb_f5_3way_revisit_base_v1")
    .select("season,game_pk,game_date,home_f5,away_f5,y_class,payload,outcome_lineage,development_class,research_only",{count:"exact"})
  ...
  return json({
    contract:CONTRACT,researchOnly:true,sourceTable:"public.mlb_f5_3way_revisit_base_v1",
