...
const CONTRACT="MLB_F1_ML_REVISIT_GITHUB_EXPORT/1.0.0";
const AUDIENCE="supabase-f1-ml-revisit";
const REPOSITORY="jlebronmarty-netizen/pick-analyzer";
const ALLOWED_REF="refs/heads/research/f1-ml-revisit-20260919-clean2";
...
Deno.serve(async(req:Request)=>{
  ...
  const {data,error,count}=await db
    .from("mlb_f1_ml_revisit_base_v1")
    .select("season,game_pk,game_date,home_f1,away_f1,y_home,payload,outcome_lineage,development_class,research_only",{count:"exact"})
    .order("game_date",{ascending:true})
    .order("game_pk",{ascending:true})
    .range(offset,offset+limit-1);
  if(error) return json({error:"DB_READ_FAILED",detail:error.message},500);
  return json({
    contract:CONTRACT,researchOnly:true,
    sourceTable:"public.mlb_f1_ml_revisit_base_v1",
    historicalMaxGameDate:"2026-09-18",
    returned:data?.length??0,totalRows:count??null,offset,limit,rows:data??[],
    forwardIncluded:false,oddsApiHistoricalCreditsConsumed:0,
