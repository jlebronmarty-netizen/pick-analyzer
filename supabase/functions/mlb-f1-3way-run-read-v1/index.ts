...
const CONTRACT="MLB_F1_3WAY_REVISIT_GITHUB_EXPORT/1.0.0";
const AUDIENCE="supabase-f1-3way-revisit";
const REPOSITORY="jlebronmarty-netizen/pick-analyzer";
const ALLOWED_REF="refs/heads/research/f1-3way-revisit-20260919";
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const expected=REPOSITORY+"/.github/workflows/mlb-f1-3way-revisit-v1.yml@"+ALLOWED_REF;
  ...
  const {data,error,count}=await db.from("mlb_f1_3way_revisit_base_v1")
    .select("season,game_pk,game_date,home_f1,away_f1,y_class,payload,outcome_lineage,development_class,research_only",{count:"exact"})
  ...
  return json({
    contract:CONTRACT,researchOnly:true,sourceTable:"public.mlb_f1_3way_revisit_base_v1",
