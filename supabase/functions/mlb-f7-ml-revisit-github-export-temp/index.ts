...
const CONTRACT="MLB_F7_ML_REVISIT_GITHUB_EXPORT/1.0.0";
const AUDIENCE="supabase-f7-ml-revisit";
const REPOSITORY="jlebronmarty-netizen/pick-analyzer";
const ALLOWED_REF="refs/heads/research/f7-ml-revisit-20260919";
...
Deno.serve(async(req:Request)=>{
  ...
  const {data,error,count}=await db
    .from("mlb_f7_ml_revisit_base_v1")
    .select("season,game_pk,game_date,home_f7,away_f7,y_home,payload,outcome_lineage,development_class,research_only",{count:"exact"})
  ...
  return json({
    contract:CONTRACT,
    researchOnly:true,
    sourceTable:"public.mlb_f7_ml_revisit_base_v1",
