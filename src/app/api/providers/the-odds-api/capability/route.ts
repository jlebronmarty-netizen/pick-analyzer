...
import { getTheOddsApiCapability } from '@/services/the-odds-api-maximum-utilization.service'

export async function GET(request: NextRequest) {
  const id = requestId(request)
  try {
    return apiOk(
      await getTheOddsApiCapability({
    ...
    return apiError({ id, code: 'INTERNAL_ERROR', message: errorMessage(error, 'Unknown The Odds API capability error') })
  }
}
