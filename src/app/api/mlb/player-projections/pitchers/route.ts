...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return apiOk({ ...data, projections: data.pitcherProjections, scope: 'pitchers' }, id)
  } catch (error) {
    return apiError({ id, code: 'INTERNAL_ERROR', message: errorMessage(error, 'Unknown MLB pitcher projection error') })
  }
}
