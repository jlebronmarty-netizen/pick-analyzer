...
export async function POST(request: NextRequest) {
  ...
  if (!authorized(request)) {
    return apiError({
      id,
      code: 'UNAUTHORIZED',
      message: 'Unauthorized historical import resume request.',
      status: 401,
    })
  }
  ...
  try {
    ...
    console.error('SportsDataIO historical import resume error:', {
      requestId: id,
      error,
    })
    ...
    return apiError({
      id,
      code: 'INTERNAL_ERROR',
      message: errorMessage(error, 'Unknown historical import resume error'),
    })
  }
}
