...
type ApiData = {
  ...
  performanceTimeline?: Array<{ label: string; generated?: number; productionEligible?: number; productionSettled?: number; productionPending?: number; nonProductionRows?: number; wins?: number; losses?: number; pushes?: number; record: string; accuracy: number | null; displayAccuracy?: string; predictions: number; zeroSampleMessage?: string | null }>
  aiBrain: {
...
function TimelineSection({ timeline, trust, grade }: { timeline: NonNullable<ApiData['performanceTimeline']>; trust: number | null; grade: string }) { return <Section title="Performance Timeline"><div className="grid gap-3 md:grid-cols-5">{timeline.map((item) => <div key={item.label} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3"><p className="font-black text-white">{item.label}</p><p className="mt-2 text-sm text-slate-400">Generated {item.generated ?? item.predictions}</p><p className="mt-1 text-sm text-slate-300">Production Eligible {item.productionEligible ?? 0}</p><p className="mt-1 text-sm text-slate-300">Production Settled {item.productionSettled ?? 0}</p><p className="mt-1 text-sm text-slate-300">Wins {item.wins ?? 0} / Losses {item.losses ?? 0} / Pushes {item.pushes ?? 0}</p><p className="mt-1 text-sm font-black text-white">Accuracy {item.displayAccuracy ?? displayNumber(item.accuracy, '%')}</p>{item.nonProductionRows ? <p className="mt-2 text-xs leading-5 text-slate-500">Non-production rows {item.nonProductionRows}</p> : null}{item.zeroSampleMessage ? <p className="mt-2 text-xs leading-5 text-slate-500">{item.zeroSampleMessage}</p> : null}</div>)}</div><p className="mt-3 text-sm text-slate-500">Current trust {displayNumber(trust)} / grade {grade}. Accuracy and Production Settled use production-evaluable settled rows only; generated rows can include analyzed, quarantined or blocked rows.</p></Section> }
