...
type ApiData = {
  ...
  performanceTimeline?: Array<{ label: string; generated?: number; productionSettled?: number; wins?: number; losses?: number; pushes?: number; record: string; accuracy: number | null; displayAccuracy?: string; predictions: number; zeroSampleMessage?: string | null }>
  aiBrain: {
...
function dateTime(value: string | null | undefined) { if (!value) return 'Not available'; const parsed = new Date(value); return Number.isFinite(parsed.getTime()) ? parsed.toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : 'Not available' }
function dateOnly(value: string | null | undefined) { if (!value) return 'Not available'; const parsed = new Date(value); return Number.isFinite(parsed.getTime()) ? parsed.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : 'Not available' }
...
export default function PerformanceProductClient() {
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.search); setSportKey(sportAlias(params.get('sport') ?? params.get('sportKey'))) }, [])
  useEffect(() => { const params = new URLSearchParams(window.location.search); sportKey === 'all' ? params.delete('sport') : params.set('sport', sportKey); window.history.replaceState(null, '', `${window.location.pathname}${params.toString() ? `?${params}` : ''}`) }, [sportKey])
        ...
        <section className="rounded-lg border border-emerald-500/20 bg-slate-900/80 p-5 md:p-6">
          <div className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr] lg:items-end">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">Trust reflects sample size, calibration, data quality and prediction stability. It is not a win probability or a betting recommendation.</p>
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {selectedSport?.sportKey === 'basketball_bsn' ? <Badge tone="yellow">SHADOW / PREVIEW</Badge> : null}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <MetricCard label="Readiness" value={displayNumber(data.aiBrain.selected.readiness.score)} sub={labelize(data.aiBrain.selected.readiness.status)} />
              <MetricCard label="Sample" value={`${data.aiBrain.selected.sampleSize}`} sub={`Updated ${dateTime(data.publicView.lastUpdate)}`} />
            </div>
          </div>
        </section>
...
function AllSportsSummary({ sports, selected }: { sports: SportSummary[]; selected: string }) {
  return <Section title="All Sports"><div className="grid gap-3 lg:grid-cols-2">{sports.map((sport) => { const modelState = sport.sportKey === 'basketball_bsn' ? 'Shadow / Preview' : sport.productionReady ? 'Production Stable' : 'Insufficient Data'; return <article key={sport.sportKey} className={`rounded-lg border p-4 ${selected === sport.sportKey ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-slate-800 bg-slate-950/60'}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-lg font-black text-white">{sport.label}</p><p className="mt-1 text-sm text-slate-400">{modelState}</p></div><Badge tone={statusTone(sport.trust.trustLabel)}>{sport.trust.trustLabel}</Badge></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm md:grid-cols-4"><Mini label="Grade" value={sport.dailyReportCard.overallGrade} /><Mini label="Trust" value={displayNumber(sport.trust.trustScore)} /><Mini label="Accuracy" value={sport.metrics.accuracy === null ? 'N/A' : `${sport.metrics.accuracy}%`} /><Mini label="Sample" value={`${sport.metrics.settled} settled`} /><Mini label="Brier" value={displayNumber(sport.metrics.brierScore)} /><Mini label="Calibration Error" value={displayNumber(sport.metrics.calibrationError)} /><Mini label="Readiness" value={displayNumber(sport.readiness.readinessScore)} /><Mini label="Trend" value={labelize(sport.trust.trustStatus)} /></div>{sport.metrics.settled === 0 ? <p className="mt-3 text-sm text-slate-400">No eligible predictions settled in this period.</p> : null}</article> })}</div></Section>
}

function SportSpecific({ sport, timeline }: { sport: SportSummary; timeline: NonNullable<ApiData['performanceTimeline']> }) {
  const categories = [['Official Picks', sport.metrics.officialAccuracy], ['AI Leans', sport.metrics.aiLeanAccuracy], ['Watchlist', sport.metrics.watchlistAccuracy], ['Avoid', sport.metrics.avoidAccuracy], ['Shadow', sport.metrics.shadowAccuracy]]
  return <Section title={`${sport.shortLabel} View`} action={<Badge tone={sport.sportKey === 'basketball_bsn' ? 'yellow' : sport.productionReady ? 'green' : 'gray'}>{sport.sportKey === 'basketball_bsn' ? 'SHADOW / PREVIEW' : sport.productionReady ? 'Production Stable' : 'Insufficient Data'}</Badge>}><div className="grid gap-4 lg:grid-cols-[1fr_1fr]"><div className="grid grid-cols-2 gap-3 md:grid-cols-4"><Mini label="Predictions" value={`${sport.metrics.predictions}`} /><Mini label="Correct" value={`${sport.metrics.correct}`} /><Mini label="Incorrect" value={`${sport.metrics.incorrect}`} /><Mini label="Accuracy" value={sport.metrics.accuracy === null ? 'N/A' : `${sport.metrics.accuracy}%`} /><Mini label="Brier" value={displayNumber(sport.metrics.brierScore)} /><Mini label="Reliability" value={displayNumber(sport.trust.trustConfidence)} /><Mini label="Calibration Error" value={displayNumber(sport.metrics.calibrationError)} /><Mini label="Readiness" value={displayNumber(sport.readiness.readinessScore)} /></div><div className="rounded-lg border border-slate-800 bg-slate-950/60 p-4"><p className="text-sm font-black text-white">Category Performance</p><div className="mt-3 grid gap-2">{categories.map(([label, value]) => <div key={label} className="flex items-center justify-between gap-3 rounded-md bg-slate-900/80 px-3 py-2 text-sm"><span className="text-slate-300">{label}</span><span className="font-black text-white">{value === null ? 'N/A' : `${value}%`}</span></div>)}</div>{sport.sportKey === 'basketball_bsn' ? <p className="mt-3 text-sm text-amber-200">No betting recommendations enabled.</p> : null}{sport.sportKey === 'baseball_mlb' ? <p className="mt-3 text-sm text-emerald-200">MLB remains Production Stable / Maintenance Mode.</p> : null}</div></div><div className="mt-4 grid gap-2 md:grid-cols-5">{timeline.slice(0, 5).map((item) => <Mini key={item.label} label={item.label} value={`${item.productionSettled ?? 0} settled / ${item.displayAccuracy ?? 'N/A'}`} />)}</div></Section>
}
