    ...
    <main className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto max-w-7xl space-y-8 p-4 md:p-8">
        <header className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
          ...
          <h1 className="mt-2 text-4xl font-black">AI Bet Finder</h1>
          ...
          <div className="mt-6 flex flex-col gap-3 md:flex-row">
            <input
              ...
              className="min-h-12 flex-1 rounded-2xl border border-slate-700 bg-slate-950 px-4 text-sm text-white"
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {examples.map((example) => (
              <button
                ...
                className="rounded-full border border-slate-700 bg-slate-950 px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-800"
              >
                {example}
              </button>
            ))}
        ...
        {data ? (
          <>
            <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
              ...
              <h2 className="mt-2 text-2xl font-black">{data.summary}</h2>
              <div className="mt-5 grid gap-3 md:grid-cols-4">
                <Meta label="Board" value={data.meta?.boardMode ?? 'n/a'} />
                <Meta label="Scanned" value={String(data.meta?.candidatesScanned ?? 0)} />
                ...
                <article key={item.id} className="rounded-3xl border border-slate-800 bg-slate-900/60 p-5">
                  <div className="grid gap-5 lg:grid-cols-[1fr_1fr_1fr]">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-400">{item.market} | {item.matchup}</p>
            ...
            {data.explanation ? (
              <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
                <h2 className="text-2xl font-black">{data.explanation.title}</h2>
            ...
            <details className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
              ...
              <pre className="mt-4 overflow-auto rounded-xl bg-slate-950 p-4 text-xs text-slate-300">{JSON.stringify(data, null, 2)}</pre>
            </details>
          </>
        ) : null}
      </div>
