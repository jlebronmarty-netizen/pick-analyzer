...
type MlbFeatureStoreResponse = {
  ...
  summary: {
    ...
    storedTeamStatsRows: number
    storedGameResultsRows: number
  ...
  compatibility: {
    ...
    changesLegacyMlbPredictionGeneration: boolean
...
export default function MlbFeatureStoreIntegrationPanel() {
  const [data, setData] = useState<MlbFeatureStoreResponse | null>(null)
  ...
  async function load() {
    try {
      ...
      const response = await fetch('/api/mlb/features/store', {
        cache: 'no-store',
      })
      const json = await response.json()

      if (!response.ok || !json.success) {
        throw new Error(json.error ?? 'Unable to load MLB Feature Store')
      }
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : 'Unable to load MLB Feature Store'
      )
    } finally {
      setLoading(false)
    }
  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
            Verifies MLB can use Feature Store Core, the Multi-Sport Feature
            Registry and the Shared Prediction SDK while keeping pitcher,
            lineup, weather and park gaps explicit.
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <Stat label="Definitions" value={data?.summary.mlbDefinitions ?? 0} />
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-2">
        <div className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4">
          ...
          <div className="mt-4 grid gap-3">
            <MiniRow
              label="Feature Store Core"
              value={data?.compatibility.usesFeatureStoreCore ? 'yes' : 'no'}
            />
            <MiniRow
              label="Feature Registry"
              value={data?.compatibility.usesMultiSportFeatureRegistry ? 'yes' : 'no'}
            />
            <MiniRow
              label="Shared Prediction SDK"
              value={data?.compatibility.usesSharedSportPredictionSdk ? 'yes' : 'no'}
            />
            <MiniRow
              label="Changes Legacy MLB"
              value={data?.compatibility.changesLegacyMlbPredictionGeneration ? 'yes' : 'no'}
            />
            <MiniRow
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4">
          ...
          <div className="mt-4 grid gap-3">
            <MiniRow
              ...
              value={String(data?.summary.storedTeamStatsRows ?? 0)}
            />
            <MiniRow
              label="Game Results Rows"
              value={String(data?.summary.storedGameResultsRows ?? 0)}
            />
            <MiniRow
              label="Prediction Rows"
              value={String(data?.summary.storedPredictionRows ?? 0)}
            />
            <MiniRow
              label="Compatible Snapshots"
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-2">
        <div className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4">
          <p className="text-sm font-black text-white">Missing MLB Domains</p>
          <div className="mt-4 grid gap-3">
