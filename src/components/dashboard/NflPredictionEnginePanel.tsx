...
export default function NflPredictionEnginePanel() {
  const [data, setData] = useState<NflPredictionResponse | null>(null)
  ...
  async function load() {
    try {
      ...
      const response = await fetch('/api/nfl/predictions', {
        cache: 'no-store',
      })
      const json = await response.json()

      if (!response.ok || !json.success) {
        throw new Error(json.error ?? 'Unable to load NFL Prediction Engine')
      }
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : 'Unable to load NFL Prediction Engine'
      )
    } finally {
      setLoading(false)
    }
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
            Deterministic moneyline, spread, total and first-half previews using
            the Shared Prediction SDK and NFL Feature Store contracts. These are
