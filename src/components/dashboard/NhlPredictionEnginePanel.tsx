...
export default function NhlPredictionEnginePanel() {
  const [data, setData] = useState<NhlPredictionResponse | null>(null)
  ...
  async function load() {
    try {
      ...
      const response = await fetch('/api/nhl/predictions', {
        cache: 'no-store',
      })
      const json = await response.json()

      if (!response.ok || !json.success) {
        throw new Error(json.error ?? 'Unable to load NHL Prediction Engine')
      }
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : 'Unable to load NHL Prediction Engine'
      )
    } finally {
      setLoading(false)
    }
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
            Deterministic moneyline, puck-line and total previews using the
            Shared Prediction SDK and NHL Feature Store contracts. These are
        ...
        <div className="flex flex-col items-start gap-3 lg:items-end">
          ...
          <button
            type="button"
            onClick={load}
            className="rounded-xl border border-blue-500/30 bg-blue-950/30 px-4 py-2 text-sm font-bold text-blue-100 hover:bg-blue-900/40"
          >
            Refresh
          </button>
        </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-black uppercase text-white">
                  {prediction.market === 'spread' ? 'puck line' : prediction.market}
                </p>
