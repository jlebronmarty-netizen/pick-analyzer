...
export default function TennisPredictionEnginePanel() {
  const [data, setData] = useState<TennisPredictionResponse | null>(null)
  ...
  async function load() {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch('/api/tennis/predictions', { cache: 'no-store' })
      const json = await response.json()
      if (!response.ok || !json.success) {
        throw new Error(json.error ?? 'Unable to load Tennis Prediction Engine')
      }
      setData(json)
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : 'Unable to load Tennis Prediction Engine')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading && !data) {
    return (
      <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6 text-sm text-slate-400">
        Loading Tennis Prediction Engine...
      </section>
    )
  }
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
            Deterministic match-winner and totals-style previews using shared
            prediction contracts. These are architecture checks, not live
        ...
        <div className="flex flex-col items-start gap-3 lg:items-end">
          ...
          <button
            type="button"
            onClick={load}
            className="rounded-xl border border-green-500/30 bg-green-950/30 px-4 py-2 text-sm font-bold text-green-100 hover:bg-green-900/40"
          >
            Refresh
          </button>
        </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-black uppercase text-white">
                  {prediction.market === 'total' ? 'match total' : prediction.market}
                </p>
