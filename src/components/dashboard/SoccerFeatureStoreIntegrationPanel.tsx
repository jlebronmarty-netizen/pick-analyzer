...
type SoccerFeatureStoreResponse = {
  success: boolean
  status: string
  providerUsage: { externalProviderCallsMade: number }
  summary: {
    soccerDefinitions: number
...
export default function SoccerFeatureStoreIntegrationPanel() {
  const [data, setData] = useState<SoccerFeatureStoreResponse | null>(null)
  ...
  async function load() {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch('/api/soccer/features/store', { cache: 'no-store' })
      const json = await response.json()
      if (!response.ok || !json.success) {
        throw new Error(json.error ?? 'Unable to load Soccer Feature Store')
      }
      setData(json)
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : 'Unable to load Soccer Feature Store')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
            Verifies soccer can use shared feature contracts while draw-aware,
            league, lineup and injury gaps remain explicit.
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <Stat label="Definitions" value={data?.summary.soccerDefinitions ?? 0} />
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4">
          <p className="text-sm font-black text-white">Missing Soccer Domains</p>
          <div className="mt-4 grid gap-3">
