...
type PropComparisonApiData = {
  ...
  supportedMarkets: Array<{ marketKey: string; displayName: string; shortLabel?: string; family?: string; storedRows?: number; availableBookmakers?: string[] }>
  marketSummary: Array<{ marketKey: string; displayName: string; family: string; storedRows: number; comparableRowsForProjectionEvents: number; availableBookmakers: string[]; status: string }>
  summary: { marketRowsEvaluated: number; comparisonsGenerated: number; sportsbooks: number; noPropAvailable: number }
  coverage: { currentStoredRows: number; sportsbooks: string[]; historicalDepth: string; currentStoredRowsByMarket?: Record<string, number> }
...
const defaultPropMarkets = [
  { marketKey: 'pitcher_outs_recorded', displayName: 'Pitcher Outs', shortLabel: 'Pitcher Outs', family: 'pitcher', storedRows: 0, availableBookmakers: [] },
]
...
function PropComparisonPanel({ comparisons, marketLabel }: { comparisons: PropComparison[]; marketLabel: string }) {
  if (!comparisons.length || comparisons.every((comparison) => comparison.bestStatus === 'NO_PROP_AVAILABLE')) {
    return (
      <div className="mt-4 rounded-lg border border-slate-800 bg-slate-950/70 p-3">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-300">No current {marketLabel.toLowerCase()} sportsbook line is stored for this player. Projection Only. No recommendation.</p>
      </div>
    )
  }
  return (
...
function PitcherOutsCard({ item, comparisons, marketLabel }: { item: PitcherOutsProjection; comparisons: PropComparison[]; marketLabel: string }) {
  ...
  return (
    <article className="rounded-lg border border-slate-800 bg-slate-900/75 p-4">
      ...
      <PropComparisonPanel comparisons={comparisons} marketLabel={marketLabel} />
...
export default function MlbPlayerProjectionPageClient() {
  ...
  const [activePropMarket, setActivePropMarket] = useState('pitcher_outs_recorded')
  ...
  useEffect(() => {
    ...
    fetch(`/api/mlb/player-props?limit=500&market=${encodeURIComponent(activePropMarket)}`, { cache: 'no-store' })
      .then((response) => {
      ...
      .catch(() => alive && setPropComparisonData({
        ...
        selectedMarket: activePropMarket,
        supportedMarkets: defaultPropMarkets,
        marketSummary: [],
  ...
  const propMarkets = propComparisonData?.supportedMarkets?.length ? propComparisonData.supportedMarkets : defaultPropMarkets
  const activeMarketMeta = propMarkets.find((market) => market.marketKey === activePropMarket) ?? propMarkets[0]
  const activeMarketSummary = propComparisonData?.marketSummary?.find((market) => market.marketKey === activePropMarket)
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-6 text-white md:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        {active === 'pitcherOuts' && (
          <div className="mt-4 grid gap-3 rounded-lg border border-slate-800 bg-slate-900/75 p-4 lg:grid-cols-[minmax(240px,360px)_1fr]">
            <label className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">
              Prop Market
              <select value={activePropMarket} onChange={(event) => setActivePropMarket(event.target.value)} className="mt-2 block h-10 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 text-sm font-bold text-white outline-none focus:border-emerald-400">
                {propMarkets.map((market) => (
                  <option key={market.marketKey} value={market.marketKey}>{market.shortLabel ?? market.displayName}</option>
                ))}
              </select>
            </label>
            <div className="grid gap-3 sm:grid-cols-3">
              <p className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-300">Stored Rows <span className="block font-black text-white">{activeMarketSummary?.storedRows ?? activeMarketMeta?.storedRows ?? 0}</span></p>
              <p className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-300">Bookmakers <span className="block font-black text-white">{activeMarketSummary?.availableBookmakers?.length ?? activeMarketMeta?.availableBookmakers?.length ?? 0}</span></p>
              <p className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-300">Status <span className="block font-black text-white">{labelize(activeMarketSummary?.status ?? 'NO_PROP_AVAILABLE')}</span></p>
            </div>
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500 lg:col-span-2">
              Only genuine stored sportsbook markets are compared. Projection Only / No recommendation.
            </p>
          </div>
        )}
        <section className="mt-5 grid gap-4 lg:grid-cols-2">
          {active === 'pitcherOuts' && pitcherOutRows.length ? pitcherOutRows.map((item) => <PitcherOutsCard key={item.projectionId} item={item} comparisons={comparisonsByProjection.get(item.projectionId) ?? []} marketLabel={activeMarketMeta?.displayName ?? 'Player Prop'} />) : active === 'pitcherOuts' ? (
