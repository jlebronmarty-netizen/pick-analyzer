...
export default function NhlFeatureStoreIntegrationPanel() {
  const [data, setData] = useState<NhlFeatureStoreResponse | null>(null)
  ...
  async function load() {
    try {
      ...
      const response = await fetch('/api/nhl/features/store', {
        cache: 'no-store',
      })
      const json = await response.json()

      if (!response.ok || !json.success) {
        throw new Error(json.error ?? 'Unable to load NHL Feature Store')
      }
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : 'Unable to load NHL Feature Store'
      )
    } finally {
      setLoading(false)
    }
  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
            Verifies NHL can use shared feature contracts while goalie,
            injury, rest and special-teams gaps remain explicit.
          </p>
        </div>

        <div className="flex flex-col items-start gap-3 lg:items-end">
          <p
            className={`text-sm font-black uppercase ${statusClass(
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <Stat label="Definitions" value={data?.summary.nhlDefinitions ?? 0} />
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-2">
        <div className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4">
          <p className="text-sm font-black text-white">Missing NHL Domains</p>
          <div className="mt-4 grid gap-3">
