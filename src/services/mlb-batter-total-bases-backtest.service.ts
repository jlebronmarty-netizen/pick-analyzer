...
type ModelRow = {
  ...
  actual: number
  priorTbAllPerGame: number
  priorTbL10PerGame: number
  priorTbRateAll: number
  priorTbRateL10: number
  priorPaL10: number
  priorGames: number
  strictPriorFeature: boolean
}

const MODEL_VERSION = 'MLB_BATTER_TOTAL_BASES_RESEARCH_V1'
...
const PROP_LINES = [0.5, 1.5, 2.5, 3.5, 4.5]
...
async function fetchBatterLogs(): Promise<BatterLog[]> {
  ...
  for (let offset = 0; ; offset += PAGE_SIZE) {
    const { data, error } = await supabaseAdmin
      .from('mlb_statcast_batter_total_bases_game_mv')
      .select('season,game_pk,game_date,batter,total_bases,plate_appearances')
      .in('season', [2025, 2026])
    ...
    if (error) throw new Error(`Statcast batter total-bases game read failed: ${error.message}`)
    ...
    for (const row of page) {
      output.push({
        ...
        totalBases: requiredNumber(row.total_bases, 'total_bases'),
      ...
      if (recent.length >= 10 && priorPa > 0 && strict) {
        ...
        if (recentPa > 0 && priorGames > 0) {
          rows.push({
            ...
            actual: log.totalBases,
            priorTbAllPerGame: priorTb / priorGames,
            priorTbL10PerGame: recentTb / recentGames,
            priorTbRateAll: priorTb / priorPa,
            priorTbRateL10: recentTb / recentPa,
...
function rawScore(row: ModelRow, alpha: number) {
  const rate = alpha * row.priorTbRateL10 + (1 - alpha) * row.priorTbRateAll
...
function selectAlpha(train: ModelRow[], validation: ModelRow[]) {
  ...
  if (!selected) throw new Error('No valid batter total-bases alpha candidate')
  return {
    selected,
...
function baselineMetrics(rows: ModelRow[]) {
  return {
    priorAllPerGame: metrics(rows.map((row) => ({ actual: row.actual, predicted: row.priorTbAllPerGame }))),
    priorL10PerGame: metrics(rows.map((row) => ({ actual: row.actual, predicted: row.priorTbL10PerGame }))),
  }
}
...
export async function runMlbBatterTotalBasesBacktest() {
  ...
  if (!final2025Fit) throw new Error('Unable to fit final 2025 batter total-bases model')
  ...
  return {
    ...
    design: {
      ...
      labelDefinition: '1*single + 2*double + 3*triple + 4*home_run from Statcast terminal events',
    ...
    selected: {
      alpha,
      interpretation: 'alpha weights last-10 total-bases per PA; 1-alpha weights full prior-season total-bases per PA before multiplying by prior L10 plate-appearance workload',
      trainFit,
      final2025Fit,
    ...
    safety: {
      ...
      nextGate: 'Forward shadow evaluation against frozen pregame batter total-bases lines and lineup-confirmed opportunity context',
    },
  }
}
