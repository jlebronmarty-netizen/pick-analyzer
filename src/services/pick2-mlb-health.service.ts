...
async function readRuntimeBudget() {
  try {
    const response=await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/mlb-runtime-state`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`},body:JSON.stringify({op:'inspect'}),cache:'no-store',signal:AbortSignal.timeout(10000),redirect:'error'})
    const body=await response.json()
    if(!response.ok||body.status!=='PASS'||body.protocol!=='MLB_R6_FENCED_RUNTIME_V1')return null
    const b=body.result?.operationalBudget
    // Explicit public projection. Never return raw runtime rows or headers.
    return {edgeVersion:Number.isInteger(body.result?.edgeVersion)?body.result.edgeVersion:null,budget:b?.version==='MLB_ODDS_OPERATIONAL_BUDGET_V1'?{version:b.version,activation:b.activation,day:b.day,timezone:b.timezone,cap:b.cap,perRunCap:b.perRunCap,consumed:b.consumed,remaining:b.remaining,observedCredits:b.observedCredits,unknownCreditRequests:b.unknownCreditRequests}:null}
  } catch {return null}
}
...
export async function getMlbOperationalHealth() {
  ...
  const [runRead,missionRead,leaseRead,runtimeRead]=await Promise.all([
  ...
  const configured=automation.activation==='ENABLED'
  const failed=durable?.status==='FAILED'&&durable.disposition!=='TERMINAL_PARTIAL_PRESERVED'
  const resultStatus=durable?.result && typeof durable.result==='object'&&!Array.isArray(durable.result)?durable.result.status:null
  const observedHealth=!configured?'DISABLED':failed?'BLOCKED_FAILED_RUN':!runtimeRead?'RUNTIME_UNAVAILABLE':runtimeRead.budget?.remaining===0||resultStatus==='ODDS_BUDGET_EXHAUSTED'?'DEGRADED_ODDS_BUDGET_EXHAUSTED':durable?.status==='COMPLETE'&&Date.now()-Date.parse(durable.updated_at)<=30*60000?'OBSERVED_RECENT_COMPLETION':durable?.status==='RUNNING'?'RUNNING':'NO_RECENT_COMPLETION_OBSERVED'
  const deployment={webDeploymentSha:process.env.VERCEL_GIT_COMMIT_SHA??null,edgeVersion:runtimeRead?.edgeVersion??null,frozenExecutionPackageSha:durable?.package_sha??null}
  const budgets={historicalCertification:{version:'MLB_OPERATIONAL_MISSION',consumed:missionOdds??null,cap:20,status:'IMMUTABLE_HISTORY'},operationalDaily:runtimeRead?.budget??{version:'MLB_ODDS_OPERATIONAL_BUDGET_V1',activation:runtimeRead?'NOT_ACTIVE':'UNVERIFIED'}}
  ...
  return { asOf: view.asOf, operatingDate: view.date, champion: MLB_CHAMPION, featureSet: 'MLB_ML_FEATURE_SET_V1', featureCount: 76, policy: MLB_POLICY,
    ...
    deployment,budgets,
    accountingNote: durable?'Historical certification requests and operational daily requests are separate. Provider credits are reported only when observed. Business DML excludes runtime coordination updates.':'Last published certification accounting; live durable runtime accounting is unavailable.',
    ...
    freshness: view.games.map(g => ({ gamePk: g.gamePk, gameEvidenceAt: g.evidenceAt, predictionAt: g.predictionAt })), automation: { activation: automation.activation, dryCertification: automation.dryCertification, reason: 'Configuration is separate from observed runtime health.',observedHealth }, settlement: performance.status, warnings: view.warnings, readProviderCalls: 0, readProductionDml: 0 }
}
