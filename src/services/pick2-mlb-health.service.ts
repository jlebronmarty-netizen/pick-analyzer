...
export async function getMlbOperationalHealth() {
  ...
  const current=durable?{runId:durable.run_id,packageSha:durable.package_sha,date:durable.run_date,asOf:durable.run_as_of,status:durable.status,stage:durable.stage,completed:durable.completed,eligibleScope:durable.scope,blocked:durable.blocked,result:durable.result,updatedAt:durable.updated_at,leaseActive:Date.parse(leaseRead.data?.[0]?.lease_expires_at??'')>Date.now()}:latestCoordinator
  return { asOf: view.asOf, operatingDate: view.date, champion: MLB_CHAMPION, featureSet: 'MLB_ML_FEATURE_SET_V1', featureCount: 76, policy: MLB_POLICY,
    currentRun: current, currentRunState: durable?.status??(telemetry.error ? 'COORDINATOR_TELEMETRY_UNAVAILABLE' : latestCoordinator ? latestCoordinator.status : 'NO_ACTIVE_COORDINATOR_TELEMETRY'), lastPublishedRun: mission.providerAccounting.runs.at(-1) ?? null,
