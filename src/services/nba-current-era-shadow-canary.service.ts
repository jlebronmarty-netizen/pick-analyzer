...
export type NbaCurrentEraShadowMode = 'dry-run' | 'write'
...
function buildShadowPredictionRow({
  event,
  odds,
  generatedAt,
  modelPrediction,
  ...
  return {
    ...
    edge: modelPrediction.edge,
    ev: modelPrediction.ev,
...
export async function runNbaCurrentEraShadowCanary({
  ...
  const [{ data: odds, error: oddsError }, existingLogicalKeys] = await Promise.all([
    supabaseAdmin
  ...
  const candidates = currentEvents.flatMap((event) => {
    ...
    if (!eventOdds.length) {
      return [evaluateNbaCurrentEraShadowCandidate({ event, odds: null, generatedAt, existingLogicalKeys })]
    }
    return eventOdds.map((oddsRow) =>
      evaluateNbaCurrentEraShadowCandidate({ event, odds: oddsRow, generatedAt, existingLogicalKeys })
    )
  })
  ...
  if (mode === 'write' && eligibleCandidates.length > 0) {
    const modelResult = await generateNbaPredictions({ persist: false, limit })
    const rows = eligibleCandidates.map((candidate) => {
      ...
      const modelPrediction = modelResult.predictions.find(
        (prediction) =>
          prediction.gameId === candidate.eventId &&
          prediction.market === candidate.market &&
          prediction.team === candidate.selection &&
          prediction.sportsbook === candidate.sportsbook &&
          (prediction.line ?? null) === candidate.line
      )
      if (!modelPrediction) {
        throw new Error('MISSING_REQUIRED_FEATURES: canonical NBA prediction engine did not produce the exact eligible market identity')
      }
      return buildShadowPredictionRow({ event, odds: oddsRow, generatedAt, modelPrediction })
    })
    const save = await savePredictionHistory(rows)
    inserted = save.saved ?? 0
  } else if (mode === 'write') {
    reused = candidates.filter((candidate) => candidate.skipReasons.includes('ALREADY_EXISTS')).length
  }

  const after = mode === 'write'
    ? await supabaseAdmin
        .from('prediction_history')
        .select('id', { count: 'exact', head: true })
        .eq('sport_key', NBA_SPORT_KEY)
  ...
  return {
    ...
    classification:
      eligibleCandidates.length > 0
        ? 'NBA_03A_BLOCK5_SAFE_CANARY_CERTIFIED_READY_FOR_FIRST_SHADOW'
        : 'NBA_03A_BLOCK5_SAFE_CANARY_CERTIFIED_WAITING_FOR_CURRENT_DATA',
  }
}
