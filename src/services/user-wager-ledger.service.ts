...
export async function authenticateUserWagerRequest(request: Request): Promise<AuthContext> {
  ...
  if (!token) throw Object.assign(new Error('Authentication required for remote wager persistence.'), { status: 401 })
...
function boundedMoney(value: unknown, field: string, min = 0) {
  const next = Number(value)
  if (!Number.isFinite(next) || next < min) throw Object.assign(new Error(`Invalid ${field}.`), { status: 400 })
...
function enumValue<T extends string>(value: unknown, allowed: readonly T[], fallback: T, field: string) {
  ...
  if (!allowed.includes(next)) throw Object.assign(new Error(`Invalid ${field}.`), { status: 400 })
  return next
}
...
function normalizeLegs(input: unknown): UserWagerLegInput[] {
  ...
  if (!legs.length) throw Object.assign(new Error('At least one wager leg is required.'), { status: 400 })
  return legs.slice(0, 12).map((leg) => {
    const selection = text(leg.selection)
    if (!selection) throw Object.assign(new Error('Each wager leg requires a selection.'), { status: 400 })
    return {
...
export async function createUserWager(auth: AuthContext, input: unknown) {
  ...
  if (!clientCreatedId) throw Object.assign(new Error('clientCreatedId is required for idempotent creation.'), { status: 400 })
...
export async function getUserWager(auth: AuthContext, id: string) {
  const { data, error } = await auth.client
    ...
    .maybeSingle()
  if (error) throw error
  if (!data) throw Object.assign(new Error('User wager not found.'), { status: 404 })
